...
import datetime, json, time, simplejson
from array import *
...
def getjquerystudents(request):
    ...
    return render_to_response('students/search.html', context, RequestContext(request))

def getcourses(request):
    sql = 'SELECT exam, txt FROM exam_table WHERE lbl1 = \'PLACEMENT\' order by exam'
    courses = do_sql(sql)
    context = {'courses':courses,}
    return render_to_response('students/getcourses.html', context, RequestContext(request))
...
def prepopulatestudents(request):
    ...
    students = do_sql(sql)
    context = {'students':students,}
    return render_to_response('students/prepopulatestudents.html', context, RequestContext(request))

def addtoexamrec(request):
    ctgry = request.POST.get("courseCode", "")
    studentID = request.POST.get("student", "")
    panel = request.POST.get("panel", "")
    # before we add a student-exam record, we check to see if that student already has an exam record for the same language, indicated by the same first letter of ctgry
    ctgryFirst=ctgry[:1]
    ...
    if numRows == 0:
        ...
        sql = ( 'INSERT INTO exam_rec (ctgry, id, yr, cmpl_date, score1, site, sess, score2, score3, score4, score5, score6, self_rpt, conv_exam_no) VALUES (\'%s\', \'%s\', \'0\', \'%s\', \'99\', \'CART\', \'\', \'0\', \'0\', \'0\', \'0\', \'0\', \'N\', \'0\') ' % (ctgry, studentID, NOW))
        do_sql(sql)
    else:
        retVal = {'stat':'failed', 'exam':ctgry, 'studentID':studentID}
    return HttpResponse(json.dumps(retVal), mimetype="text/plain; charset=utf-8")
...
def removefromexamrec(request):
        ...
        panel = request.POST.get("panel", "")
        sql = ( 'DELETE FROM exam_rec WHERE id=\'%s\' AND ctgry=\'%s\'' % (studentID, courseCode))
        do_sql(sql)

        nameSQL = do_sql('SELECT firstname, lastname FROM id_rec WHERE id_rec.id = \'%s\'' % studentID)
        stuName=""
        for name in nameSQL:
            stuName = name.firstname + " " + name.lastname
        courseNames = do_sql('SELECT txt FROM exam_table WHERE exam = \'%s\' ' % courseCode)
        courseName=""
        ...
        return HttpResponse(json.dumps(retVal), mimetype="application/json; charset=utf-8")
